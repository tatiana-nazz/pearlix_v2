...
def test_complete_visit_rejects_invalid_billing_without_state_change(
    doctor_client, doctor_user, appointment_factory, visit_factory, billing_overrides
):
    ...
    response = doctor_client.post(
        ...
        _completion_payload(visit, **billing_overrides),
        format="json",
    )
...
@pytest.mark.django_db
def test_complete_visit_rolls_back_when_handoff_creation_fails(
    monkeypatch, doctor_client, doctor_user, appointment_factory, visit_factory
):
    visit = _visit_for(visit_factory, appointment_factory, doctor_user, clinical_notes="Original")
...
@pytest.mark.django_db
def test_complete_visit_conflicts_preserve_active_visit(
    doctor_client, doctor_user, appointment_factory, visit_factory
):
    visit = _visit_for(visit_factory, appointment_factory, doctor_user)
    stale_payload = _completion_payload(visit)
    stale_payload["version"] = "2020-01-01T00:00:00Z"

    stale_response = doctor_client.post(f"/api/visits/{visit.id}/complete/", stale_payload, format="json")

    assert stale_response.status_code == 409
    assert stale_response.data["code"] == "VERSION_CONFLICT"
    visit.refresh_from_db()
    assert visit.status == Visit.Status.ACTIVE
    ...
    existing_response = doctor_client.post(
        f"/api/visits/{visit.id}/complete/", _completion_payload(visit), format="json"
    )
...
@pytest.mark.django_db
def test_repeated_complete_request_cannot_create_duplicate_handoff(
    doctor_client, doctor_user, appointment_factory, visit_factory
):
    visit = _visit_for(visit_factory, appointment_factory, doctor_user)
