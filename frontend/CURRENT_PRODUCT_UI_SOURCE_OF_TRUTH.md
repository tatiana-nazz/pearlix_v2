...
- **Shell and navigation:** retain the Phase 14F.2-closed reference-derived shell and semantic token system. Use the Pearlix brand SVG, fixed-top simple arrow collapse, text-only `EN`/`AR`, no top-right account menu, and one sidebar profile destination. Staff has one Team and one Billing destination; Billing has exactly Overview and Invoices tabs. Staff/Doctor schedule and leave belong inside profile. The appointment reschedule queue is a Calendar sibling tab, not a sidebar item.
...
- **Billing, visits, and imaging:** Doctors have no global Billing or payment processing. The owning Doctor completes an active visit with final billing and receives an immediate unpaid invoice response in the same atomic workflow; no Staff approval is required. Staff owns manual invoice and payment operations, while Admin is read-only. Visits, X-ray/AI, protected media, and AI boundaries remain governed by current backend contracts and role tests.
...
The opened Active Visit workflow uses a compact patient/visit summary and one accessible tab level for notes, read-first patient context, authorized X-rays/attachments, and final billing. Save Notes is the dominant dirty-state action; completion is a separately confirmed clinical finalization that atomically completes the visit and appointment and creates the visit's immediate unpaid invoice. It preserves role ownership, protected media/stored AI boundaries, and the Doctor's no-payment/no-global-Billing boundary.
...
## Billing and patient financial workspace: Stage 6

Billing is invoice-centric with exactly Overview and Invoices primary tabs. The Overview uses backend-derived invoice KPIs, recent invoices, and a compact today summary; current dashboards contain no pending-handoff metric or queue. Invoice history rows open invoice detail, while patient links and patient-cell double click open the patient's Billing tab. Invoice detail presents required description, origin, linked patient/visit/appointment, financial summary, payment history, and an A4 print view. Staff alone creates manual invoices and manages eligible edits, payments, cancellation, and Print; after a payment, description and notes remain editable while financial fields lock. Admin is read-only with Print. Doctors have no global Billing navigation or invoice/payment capability. Patient Billing shows patient-scoped totals and invoices, with Staff-only patient-prefilled creation. Mutations await cross-workspace cache invalidation, and operational billing/dashboard queries refresh on focus and at a bounded interval. See `design_v3/STAGE_6_BILLING_PATIENT_FINANCIAL_WORKFLOW_RECORD.md`.
...
Month appointment items visibly reuse canonical semantic status tones and include localized status text. The individual Patient Profile keeps one sticky logical-start identity rail on desktop and stacks it at 1023px and below. Active Visit keeps a static patient-and-visit summary followed by exactly Visit Notes, Patient Profile, X-rays / Attachments, and Billing; Stage 6 supersedes the prior handoff wording and behavior with immediate invoice creation. The X-ray tab owns a large inline authenticated viewer, saved selection, authorized visit upload, the existing saved-X-ray AI mutation, same-canvas overlay, real viewer controls, backend-derived AI result panel, and explicit research-only/non-diagnostic language. See `design_v3/PHASE_14F4_ACTIVE_VISIT_PATIENT_RAIL_MONTH_XRAY_WORKSPACE_RECORD.md` and `design_v3/STAGE_6_BILLING_PATIENT_FINANCIAL_WORKFLOW_RECORD.md`.
