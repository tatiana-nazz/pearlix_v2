...
- **Billing, visits, and imaging:** Doctors have no global Billing or payment processing. The owning Doctor completes an active visit with final billing and atomically creates one OPEN Handoff/Bill with zero Invoices. A Handoff is the amount owed; each Invoice is one completed payment receipt, so one Handoff has zero or many Invoices. Staff creates manual Bills and issues Invoices only from eligible Handoff detail; Admin is read-only. Visits, X-ray/AI, protected media, and AI boundaries remain governed by current backend contracts and role tests.
...
Billing uses the Pearlix hierarchy `Visit → Handoff/Bill → zero or many Invoices`. Handoffs are complete financial obligations with OPEN, PARTIALLY_PAID, PAID, or CANCELLED status and backend-authoritative paid, remaining, and Invoice counts. Invoices are immutable receipts for individual completed payments and have no Patient, Currency, or Treatment selector. Overview, Handoffs, and Invoices are first-class tabs; dashboards and summaries use Handoff debt and Invoice collection metrics without combining currencies. Staff creates manual Bills, safely edits or cancels eligible zero-receipt Bills, and records payment from Handoff detail. Admin is read-only; Doctors may create a Handoff only through completing their own Visit. Patient Billing is Handoff-first and carries fixed Patient context into Staff New Bill. Mutations invalidate Handoff, Invoice, Patient Billing, Visit/Appointment, summary, and dashboard caches, with focus refresh and bounded operational refresh retained. Stage 7 supersedes the Stage 6 financial hierarchy; see `design_v3/STAGE_7_HANDOFF_BILL_INVOICE_LEDGER_RECORD.md`.
