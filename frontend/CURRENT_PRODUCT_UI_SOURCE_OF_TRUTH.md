...
- **Shell and navigation:** retain the Phase 14F.2-closed reference-derived shell and semantic token system. Use the Pearlix brand SVG, fixed-top simple arrow collapse, text-only `EN`/`AR`, no top-right account menu, and one sidebar profile destination. Staff has one Team and one Billing destination; Billing handoffs/invoices are tabs. Staff/Doctor schedule and leave belong inside profile. The appointment reschedule queue is a Calendar sibling tab, not a sidebar item.
