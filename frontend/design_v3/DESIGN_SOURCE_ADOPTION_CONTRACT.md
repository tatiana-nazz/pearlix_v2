...
Phase 14F.4 adopts the supplied Active Visit reference hierarchy without adopting its unrelated product chrome. Month items use shared appointment status tones and visible labels. Patient Profile uses a sticky logical-start identity rail above 1023px and stacked normal flow at or below it. Active Visit keeps one patient/visit summary and the exact four approved tabs visible, with a large dark inline protected X-ray canvas, saved thumbnail selection, existing authorized upload/AI mutations, same-transform overlay, backend-only result panel, and explicit research-only limitation. The 1279px, 1023px, and 767px breakpoints remain authoritative.
