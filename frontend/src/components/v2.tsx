...
export function StatusBadge({ status, label }: { status:string; label?:string }) { const text = label ?? status.split("_").join(" "); const meta = statusMeta[status] ?? { tone:"", icon:Circle }; const Icon = meta.icon; return <span className={`v2-status ${meta.tone}`} aria-label={`Status: ${text}`}><Icon size={14} strokeWidth={1.75} aria-hidden="true" />{text}</span>; }
