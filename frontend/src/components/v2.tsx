...
export function StatusBadge({ status, label }: { status:string; label?:string }) { const machineStatus = status.trim().toUpperCase().replace(/[\s-]+/g, "_"); const text = label ?? machineStatus.split("_").join(" "); const meta = statusMeta[machineStatus] ?? { tone:"neutral", icon:Circle }; const Icon = meta.icon; return <span className={`v2-status ${meta.tone}`} data-status={machineStatus} aria-label={`Status: ${text}`}><Icon size={14} strokeWidth={1.75} aria-hidden="true" />{text}</span>; }
