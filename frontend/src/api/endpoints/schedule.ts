...
const allPages = <T>(url: string, query?: QueryParams) =>
  getAllPages((page) => api.get<Page<T>>(url, { ...(query ?? {}), page }));
...
export const scheduleApi = {
  ...
  defaultShifts: () => allPages<ClinicDefaultShift>("/clinic-default-shifts/"),
  ...
  workingShifts: (query?: QueryParams) => allPages<WorkingShift>("/working-shifts/", query),
  ...
  allAvailabilityExceptions: (query?: QueryParams) => allPages<AvailabilityException>("/availability-exceptions/", query),
