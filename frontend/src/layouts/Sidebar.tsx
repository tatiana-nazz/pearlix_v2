...
export function Sidebar({ role, collapsed = false, onCollapse = () => undefined, onNavigate = () => undefined }: SidebarProps) {
  ...
  return (
    <aside className="app-sidebar">
      ...
      <button className="v2-icon-button sidebar-toggle" type="button" aria-label={collapsed ? "Expand navigation" : "Collapse navigation"} data-tooltip={collapsed ? "Expand navigation" : "Collapse navigation"} onClick={onCollapse}>{collapsed ? <PanelLeftOpen size={20} /> : <PanelLeftClose size={20} />}</button>
    </aside>
  );
}
