...
import { addDays, addMonths, calendarPeriodLabel, todayInputValue } from "../../features/appointments/utils/appointmentDates";
...
export function AppointmentsPage({ role, view }: AppointmentsPageProps) {
  ...
  const appointmentContext = calendarView
    ? calendarPeriodLabel(date, view, language === "AR" ? "ar" : "en")
    : undefined;
  ...
  return <div className="appointment-page"><PageHeader eyebrow={workspace} title={viewTitle[view]} description={appointmentContext ? `${description} · ${appointmentContext}` : description} actions={permissions.canCreate ? <Button onClick={openCreate}>{t("addAppointment")}</Button> : undefined} />
