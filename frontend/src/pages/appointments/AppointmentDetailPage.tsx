...
export function AppointmentDetailPage({ role }: AppointmentDetailPageProps) {
  ...
  if (!Number.isFinite(appointmentId) || appointmentId <= 0) {
    return <main className="appointment-detail-page"><StatePanel state="notFound" title={c.appointmentNotFound} description={c.appointmentNotFoundDescription} action={backAction} /></main>;
  }
  if (appointment.isLoading) return <main className="appointment-detail-page"><StatePanel state="loading" title={c.loading} /></main>;
  ...
  return (
    <main className="appointment-detail-page" data-role={role} data-appointment-id={item.id} data-appointment-date={appointmentDate}>
      <DetailHeader
