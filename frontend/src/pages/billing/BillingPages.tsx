...
export function BillingHandoffDetailPage({ role }: { role: UserRole }) {
  ...
  return <div className="billing-page invoice-detail-page">
    <header className="invoice-detail-header"><div><BackLink to={`/${rolePath}/billing/handoffs`}>Back to Handoffs</BackLink><div className="invoice-title-line"><h1>Bill #{item.id}</h1><StatusPill status={item.status} /></div><p>{item.description}</p></div><div className="invoice-header-actions"><Link className="button secondary" to={patientPath}>Open Patient</Link>{item.visit ? <Link className="button secondary" to={`/${rolePath}/visits/${item.visit.id}`}>Open Visit</Link> : null}</div></header>
