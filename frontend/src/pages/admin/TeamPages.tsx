...
function TeamForm({ member, onClose, onDirtyChange }: { member?: TeamMemberDetail; onClose: () => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}>
    ...
    <div className="v2-sticky-actions"><Button type="button" variant="secondary" onClick={requestClose} disabled={mutation.isPending}>Cancel</Button><Button type="submit" loading={mutation.isPending}>{member ? "Save profile" : "Create team member"}</Button></div>
  </form>;
}

export function AdminTeamListPage() {
  const [params, setParams] = useSearchParams(); const navigate = useNavigate(); const [open, setOpen] = useState(false); const [formDirty, setFormDirty] = useState(false);
  ...
  return <div className="admin-page"><PageHeaderV2 title="Team" description="Professional clinic directory. Account access is managed separately in Users & Access." action={<Button onClick={() => setOpen(true)}><UserPlus size={18} />Add team member</Button>} />
    ...
    <Modal open={open} title="Add Team Member" description="Add a clinic professional and their account." onClose={() => { setFormDirty(false); setOpen(false); }} dirty={formDirty}><TeamForm onDirtyChange={setFormDirty} onClose={() => { setFormDirty(false); setOpen(false); }} /></Modal>
  </div>;
}
