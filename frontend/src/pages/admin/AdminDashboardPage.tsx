...
export function AdminDashboardPage() {
  ...
  return <div className="dashboard-page"><PageHeaderV2 title="Clinic operations overview" description={`Local clinic date · ${today}`} action={<><Link className="v2-button secondary" to="/admin/clinic-settings">Clinic settings</Link><Link className="v2-button secondary" to="/admin/users">Users & Access</Link></>} />
    <Kpis items={[
      ...
      { label: "Today's appointments", value: data.today_appointments_count, support: "Local clinic date", to: `/admin/appointments/list?date=${today}`, icon: Icons.appointments },
      { label: "Needs reschedule", value: data.needs_reschedule_appointments_count, support: "Requires scheduling review", to: "/admin/appointments/needs-reschedule?status=NEEDS_RESCHEDULE", icon: Icons.reschedule },
    ...
    <div className="dashboard-columns"><SurfaceCard major><SectionHeading title="Needs attention" /><PreviewList initialCount={4} items={data.recent_appointments.filter((item) => item.status === "NEEDS_RESCHEDULE")} viewAll={<Link className="v2-button secondary compact" to="/admin/appointments/needs-reschedule?status=NEEDS_RESCHEDULE">View all</Link>} renderItem={(item) => <Link className="summary-row" key={item.id} to="/admin/appointments/needs-reschedule?status=NEEDS_RESCHEDULE"><span><strong>{item.patient.full_name}</strong><small>{item.reason || "Scheduling review"}</small></span><StatusBadge status={item.status} /></Link>} />{!data.recent_appointments.some((item) => item.status === "NEEDS_RESCHEDULE") ? <StatePanel state="empty" title="No appointments need attention" /> : null}</SurfaceCard>
      <SurfaceCard><SectionHeading title="Recent appointments" /><PreviewList initialCount={4} items={data.recent_appointments} viewAll={<Link className="v2-button secondary compact" to={`/admin/appointments/list?date=${today}`}>View all</Link>} renderItem={(item) => <Link className="summary-row" key={item.id} to={`/admin/appointments/list?date=${today}`}><span><strong>{item.patient.full_name}</strong><small>{item.doctor.full_name}</small></span><StatusBadge status={item.status} /></Link>} /></SurfaceCard></div>
