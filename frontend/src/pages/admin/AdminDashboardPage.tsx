...
export function AdminDashboardPage() {
  ...
  return <div className="dashboard-page"><PageHeaderV2 title={t("clinicOperations")} description={`Local clinic date · ${today}`} action={<><Link className="v2-button secondary" to="/admin/clinic-settings">Clinic settings</Link><Link className="v2-button secondary" to="/admin/users">{t("usersAccess")}</Link></>} />
    <Kpis items={[
      { label: "Active patients", value: data.total_active_patients, helper: "Active records", to: "/admin/patients?archive=active", icon: Icons.patients },
      { label: t("todayAppointments"), value: data.today_appointments_count, helper: "Local clinic date", to: `/admin/appointments/list?date=${today}`, icon: Icons.appointments },
      { label: t("needsReschedule"), value: data.needs_reschedule_appointments_count, helper: "Requires scheduling review", to: "/admin/appointments/needs-reschedule?status=NEEDS_RESCHEDULE", icon: Icons.reschedule },
      { label: "Pending handoffs", value: data.pending_billing_handoffs_count, helper: "Awaiting Staff conversion", to: "/admin/billing/handoffs?status=PENDING", icon: Icons.handoff },
      { label: "Unpaid invoices", value: data.unpaid_invoices_count, helper: "Outstanding balance", to: "/admin/billing/invoices?status=UNPAID", icon: Icons.money },
    ]} />
