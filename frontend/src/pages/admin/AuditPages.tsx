...
function AuditFiltersPanel({ filters, setFilter }: { filters: AuditFilters; setFilter: (key: Exclude<keyof AuditFilters, "page">, value: string) => void }) {
  ...
  return <section className="billing-filter-grid" aria-label={t("auditFilters")}>
...
export function AdminAuditLogListPage() {
  ...
  return <div className="admin-page"><PageHeaderV2 title={t("auditLogs")} /><SurfaceCard major><AuditFiltersPanel filters={filters} setFilter={setFilter} /><Button type="button" variant="secondary" onClick={clear}>{t("clearFilters")}</Button>{audit.isLoading ? <StatePanel state="loading" title={t("auditLogs")} /> : null}{audit.isError ? <StatePanel state="error" title={t("auditUnavailable")} description={getErrorMessage(audit.error)} action={<Button type="button" onClick={() => void audit.refetch()}>{t("retry")}</Button>} /> : null}{audit.isFetching && audit.data ? <p role="status">{t("auditRefreshing")}</p> : null}{audit.data ? <DataTableShell title={t("auditRecords")} count={audit.data.count} state={!audit.data.results.length ? <StatePanel state="empty" title={t("noAuditLogs")} /> : undefined}><table><thead><tr><th>{t("occurred")}</th><th>{t("actor")}</th><th>{t("actorRole")}</th><th>{t("action")}</th><th>{t("entityType")}</th><th>{t("entityId")}</th></tr></thead><tbody>{audit.data.results.map((item) => <ClickableRow key={item.id} onOpen={() => open(item)}><td className="bidi-isolate">{formatDateTime(item.created_at)}</td><td>{item.actor ? <bdi>{actorLabel(item.actor)}</bdi> : t("system")}</td><td>{item.actor_role ? roleLabel(item.actor_role, t) : t("system")}</td><td>{actionLabel(item.action, t)}</td><td>{entityLabel(item.entity_type, t)}</td><td><bdi>{item.entity_id}</bdi></td></ClickableRow>)}</tbody></table></DataTableShell> : null}{audit.data ? <Pagination page={filters.page} hasPrevious={Boolean(audit.data.previous)} hasNext={Boolean(audit.data.next)} onPrevious={() => setParams({ ...Object.fromEntries(params), page: String(filters.page - 1) })} onNext={() => setParams({ ...Object.fromEntries(params), page: String(filters.page + 1) })} /> : null}</SurfaceCard></div>;
}
