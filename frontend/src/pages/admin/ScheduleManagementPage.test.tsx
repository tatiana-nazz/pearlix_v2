...
describe("ScheduleManagementPage clinic operating week", () => {
  ...
  it("keeps a failed shift mutation visible and retryable", async () => {
    vi.spyOn(scheduleApi, "setDefaultShiftActive").mockRejectedValue(new Error("service unavailable"));
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    render(<QueryClientProvider client={client}><ScheduleManagementPage /></QueryClientProvider>);

    fireEvent.click(await screen.findByText("Friday template"));
