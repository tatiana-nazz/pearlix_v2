...
import type { ClinicDefaultShift, ScheduleApplyMode, WorkingShift } from "../../types/schedule";
...
export function ScheduleManagementPage() {
  ...
  const [impact, setImpact] = useState<{ action: () => void; count: number } | null>(null);
  ...
  const applyDefaults = useMutation({ mutationFn: (confirmed: boolean) => scheduleApi.applyDefault(selectedEmployee!, mode, confirmed), onSuccess: refreshEmployee, onError: (error) => { if (isImpact(error)) setImpact({ action: () => applyDefaults.mutate(true), count: Number((error as ApiClientError).details.impacted_count ?? 0) }); } });
  const copySchedule = useMutation({ mutationFn: (confirmed: boolean) => scheduleApi.copySchedule(copySource!, selectedEmployee!, mode, confirmed), onSuccess: refreshEmployee, onError: (error) => { if (isImpact(error)) setImpact({ action: () => copySchedule.mutate(true), count: Number((error as ApiClientError).details.impacted_count ?? 0) }); } });
  ...
  return <div className="schedule-page">
    ...
    {impact && <div className="conflict-banner" role="alert"><strong>{impact.count} future appointment(s) need rescheduling.</strong><span>Confirming will move each affected appointment to Needs Reschedule.</span><button className="button secondary" onClick={() => setImpact(null)}>Cancel</button><button className="button primary" onClick={() => { const action = impact.action; setImpact(null); action(); }}>Confirm shift change</button></div>}
    <div className="schedule-grid">
      ...
      <Card><h2>Employee schedules</h2><label>Employee<select value={selectedEmployee ?? ""} onChange={(event) => setSelectedEmployee(event.target.value ? Number(event.target.value) : null)}><option value="">Select an employee</option>{employeeOptions.map((employee) => <option key={employee.id} value={employee.id}>{employee.full_name} ({employee.role})</option>)}</select></label>
        ...
        {selectedShifts.isLoading ? <LoadingState title="Loading employee shifts..." /> : <ul className="schedule-list">{(selectedShifts.data?.results ?? []).map((shift: WorkingShift) => <li key={shift.id}><div><strong>{shift.name}</strong><span>{formatWeekday(shift.weekday)} | {formatClock(shift.start_time)} - {formatClock(shift.end_time)}</span></div><button className="button ghost" onClick={() => scheduleApi.setWorkingShiftActive(shift.id, shift.version, !shift.is_active).then(refreshEmployee)}>{shift.is_active ? "Deactivate" : "Activate"}</button></li>)}</ul>}</> : <EmptyState title="Select an employee to view and manage the weekly schedule." />}
      </Card>
    </div>
  </div>;
}
