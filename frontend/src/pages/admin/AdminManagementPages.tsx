...
export function AdminClinicSettingsPage() { const t = useFeatureT(); return <div className="admin-page"><PageHeaderV2 title={t("clinicSettings")} description={t("clinicSettingsHelp")} /><SurfaceCard major><ClinicForm /></SurfaceCard></div>; }
...
export function AdminAuditLogListPage() { const t = useFeatureT(); const navigate = useNavigate(); const [params, setParams] = useSearchParams(); const query = queries(params); const audit = useQuery({ queryKey: ["audit-logs", query], queryFn: () => auditApi.list(query) }); const actors = useQuery({ queryKey: ["audit-actors"], queryFn: () => usersApi.list({ page: 1 }) }); const update = (key: string, next: string) => { const value = new URLSearchParams(params); next ? value.set(key, next) : value.delete(key); if (key !== "page") value.set("page", "1"); setParams(value); }; if (audit.isLoading) return <StatePanel state="loading" title={t("auditLogs")} />; if (audit.isError || !audit.data) return <StatePanel state="error" title={t("auditUnavailable")} action={<Button onClick={() => void audit.refetch()}>{t("retry")}</Button>} />; return <div className="admin-page"><PageHeaderV2 title={t("auditLogs")} /><section className="billing-filter-grid"><Combobox label={t("actor")} value={params.get("actor_id") || ""} onChange={(v) => update("actor_id", v)} options={(actors.data?.results ?? []).map((u) => ({ value: String(u.id), label: u.full_name }))} /><SelectField label={t("actorRole")} value={params.get("actor_role") || ""} onChange={(e) => update("actor_role", e.target.value)}><option value="">{t("allRoles")}</option><option value="ADMIN">{t("admin")}</option><option value="STAFF">{t("staff")}</option><option value="DOCTOR">{t("doctors")}</option></SelectField><Field label={t("action")} value={params.get("action") || ""} onChange={(e) => update("action", e.target.value)} /><Field label={t("entityType")} value={params.get("entity_type") || ""} onChange={(e) => update("entity_type", e.target.value)} /><Field label={t("entityId")} value={params.get("entity_id") || ""} onChange={(e) => update("entity_id", e.target.value)} /><Field label={t("createdFrom")} type="datetime-local" value={params.get("created_from") || ""} onChange={(e) => update("created_from", e.target.value)} /><Field label={t("createdTo")} type="datetime-local" value={params.get("created_to") || ""} onChange={(e) => update("created_to", e.target.value)} /><Button variant="secondary" onClick={() => setParams({})}>{t("clearFilters")}</Button></section><DataTableShell title={t("auditRecords")} count={audit.data.count}>{audit.data.results.length ? <table><thead><tr><th>{t("occurred")}</th><th>{t("actor")}</th><th>{t("actorRole")}</th><th>{t("action")}</th><th>{t("entityType")}</th><th>{t("entityId")}</th></tr></thead><tbody>{audit.data.results.map((log) => <ClickableRow key={log.id} onOpen={() => navigate("/admin/audit-logs/" + log.id + (params.toString() ? "?" + params.toString() : ""))}><td className="bidi-isolate">{formatDateTime(log.created_at)}</td><td>{log.actor ? <bdi>{log.actor.full_name}</bdi> : t("system")}</td><td>{log.actor_role || t("system")}</td><td>{human(log.action)}</td><td>{human(log.entity_type)}</td><td><bdi>{log.entity_id}</bdi></td></ClickableRow>)}</tbody></table> : <StatePanel state="empty" title={t("noAuditLogs")} />}</DataTableShell><Pagination page={query.page} hasPrevious={Boolean(audit.data.previous)} hasNext={Boolean(audit.data.next)} onPrevious={() => update("page", String(query.page - 1))} onNext={() => update("page", String(query.page + 1))} /></div>; }
export function AdminAuditLogDetailPage() { const t = useFeatureT(); const id = Number(useParams().auditLogId); const [params] = useSearchParams(); const log = useQuery({ queryKey: ["audit-log", id], queryFn: () => auditApi.detail(id), enabled: id > 0 }); if (log.isLoading) return <StatePanel state="loading" title={t("auditRecord")} />; if (log.isError) return <StatePanel state="error" title={t("auditRecordUnavailable")} action={<Button onClick={() => void log.refetch()}>{t("retry")}</Button>} />; if (!log.data) return <StatePanel state="notFound" title={t("notFound")} />; const item: AuditLog = log.data; return <div className="admin-page"><Link className="inline-back-link" to={"/admin/audit-logs" + (params.toString() ? "?" + params.toString() : "")}>{t("backToAudit")}</Link><PageHeaderV2 title={t("auditRecord")} /><SurfaceCard major><dl className="detail-grid"><div><dt>{t("occurred")}</dt><dd className="bidi-isolate">{formatDateTime(item.created_at)}</dd></div><div><dt>{t("actor")}</dt><dd>{item.actor ? <bdi>{item.actor.full_name}</bdi> : t("system")}</dd></div><div><dt>{t("action")}</dt><dd>{human(item.action)}</dd></div><div><dt>{t("entityType")}</dt><dd>{human(item.entity_type)}</dd></div><div><dt>{t("entityId")}</dt><dd className="bidi-isolate">{item.entity_id}</dd></div></dl><SectionHeading title={t("metadata")} />{Object.keys(item.metadata_json).length ? <Meta value={item.metadata_json} /> : <p>{t("metadataEmpty")}</p>}</SurfaceCard></div>; }
