...
import { canAttachExternalXray, canManageExternalXray, canRunExternalXrayAi } from "../utils/xrayPermissions";
...
export function ExternalXrayDetail({ role, external }: ExternalXrayDetailProps) {
  ...
  const runErrorCode = aiErrorCode(mutations.runAi.error);
  const ai = useExternalAiResult(
    external.id,
    external.has_ai_result || runErrorCode === "AI_ANALYSIS_IN_PROGRESS",
  );
  const [attachOpen, setAttachOpen] = useState(false);
  const [discardOpen, setDiscardOpen] = useState(false);
  const canManage = canManageExternalXray(role, user?.id, external);
  const canRunAi = canRunExternalXrayAi(role, user?.id, external);
  const canAttach = canAttachExternalXray(role, user?.id, external);
  const analysisActive = isAiAnalysisActive(ai.data?.status) || mutations.runAi.isPending;
  const canStartAi = canRunAi && !analysisActive && (!ai.data || ai.data.status === "FAILED");
  const showRunAi = canRunAi && (canStartAi || analysisActive);
  const aiErrorMessage = aiRunErrorMessage(mutations.runAi.error, c);
  ...
  return <div className="xray-detail-grid"><Card><header className="xray-detail-header"><div><p className="eyebrow">External X-ray</p><h3>{xrayText(external.title || external.original_file_name)}</h3><p>External temporary record — not a patient record.</p></div><StatusPill status={external.status} /></header>
    <ProtectedXrayViewer originalEndpoint={external.file_endpoint} overlayEndpoint={external.ai_overlay_endpoint} overlayAvailable={Boolean(ai.data?.overlay_available)} originalLabel="Protected original image" originalAlt="Protected temporary external dental X-ray" />
    ...
    {canManage ? <div className="xray-detail-actions">{showRunAi ? <Button type="button" loading={mutations.runAi.isPending} disabled={analysisActive} onClick={() => mutations.runAi.mutate(external.id)}><Sparkles size={18} aria-hidden="true" />{analysisActive ? c.analyzing : ai.data?.status === "FAILED" ? c.retryAi : c.runAi}</Button> : null}{canAttach ? <Button variant="secondary" type="button" disabled={analysisActive} onClick={() => { mutations.attach.reset(); setAttachOpen(true); }}>Attach to patient</Button> : null}<Button variant="danger" type="button" disabled={analysisActive} onClick={() => { mutations.discard.reset(); setDiscardOpen(true); }}>Discard case</Button></div> : null}
  </Card><div className="xray-ai-detail-column"><AiResultPanel result={ai.data} isLoading={ai.isLoading} error={ai.error} onRetry={() => void ai.refetch()} showDisclaimer={false} /><AiAnalysisDetails result={ai.data} /></div>
