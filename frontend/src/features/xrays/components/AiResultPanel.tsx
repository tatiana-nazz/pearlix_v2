...
export function AiResultPanel({ result, isLoading, error, onRetry }: AiResultPanelProps) {
  ...
  if (!result) return <Card><div className="active-xray-ai-panel xray-ai-empty"><h3><Sparkles size={20} aria-hidden="true" />{c.aiResult}</h3><p>{c.noResult}</p><div className="ai-disclaimer" role="note"><strong>{c.researchOnly}</strong><span>{c.requiresInterpretation}</span><span>{c.notDiagnosis}</span></div></div></Card>;
  ...
  return <Card><div className="active-xray-ai-panel">
    <div className="section-header"><h3><Sparkles size={20} aria-hidden="true" />{c.aiResult}</h3><p>{c.researchOnly}</p></div>
    ...
    <div className="ai-disclaimer" role="note"><strong>{c.researchOnly}</strong><span>{c.requiresInterpretation}</span><span>{c.notDiagnosis}</span>{result.disclaimer ? <p>{result.disclaimer}</p> : null}{result.disclaimer_ar ? <p lang="ar" dir="rtl">{result.disclaimer_ar}</p> : null}</div>
  </div></Card>;
}
