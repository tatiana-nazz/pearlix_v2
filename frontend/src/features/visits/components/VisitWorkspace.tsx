...
import { Button, ClickableRow, ConfirmDialog, DataTableShell, SectionHeading, StatusBadge, SurfaceCard } from "../../../components/v2";
...
function ReadOnlyNotes({ values }: { values: ClinicalNotesValues }) {
  ...
  return <><SectionHeading title={t("clinicalNotes")} description={t("clinicalNotesReadOnly")} /><dl className="visit-notes-readonly">{notes.map(([key, label]) => <div key={key}><dt>{t(label)}</dt><dd className="bidi-isolate">{displayText(values[key]) || t("visitNotRecorded")}</dd></div>)}</dl></>;
}
...
function VisitHistory({ role, visit }: VisitWorkspaceProps) {
  ...
  return <SurfaceCard className="visit-history-card"><SectionHeading title={t("history")} description={t("clinicalHistoryDescription")} />{!rows.length ? <p>{t("noVisitsRecorded")}</p> : <DataTableShell title={t("history")} count={history.data?.count ?? rows.length}><table><thead><tr><th>{t("date")}</th><th>{t("doctor")}</th><th>{t("status")}</th><th>{t("diagnosis")}</th><th>{t("treatment")}</th></tr></thead><tbody>{rows.map((item) => <ClickableRow key={item.id} onOpen={() => navigate(`/${role.toLowerCase()}/visits/${item.id}`)}><td className="bidi-isolate">{formatDateTime(item.started_at ?? item.created_at)}</td><td><bdi>{item.doctor.full_name}</bdi></td><td><StatusBadge status={item.status} /></td><td className="bidi-isolate">{displayText(item.diagnosis) || t("visitNotRecorded")}</td><td className="bidi-isolate">{displayText(item.treatment) || t("visitNotRecorded")}</td></ClickableRow>)}</tbody></table></DataTableShell>}</SurfaceCard>;
}

function AppointmentInfo({ role, visit }: VisitWorkspaceProps) {
  const t = useFeatureT();
  return <><SurfaceCard major className="visit-appointment-card"><SectionHeading title={t("appointmentInfo")} description={t("visitDetailsDescription")} /><dl className="detail-grid"><div><dt>{t("patient")}</dt><dd><Link className="bidi-isolate" to={`/${role.toLowerCase()}/patients/${visit.patient.id}`}>{visit.patient.full_name}</Link></dd></div><div><dt>{t("doctor")}</dt><dd className="bidi-isolate">{visit.doctor.full_name}</dd></div><div className="detail-wide"><dt>{t("appointment")}</dt><dd className="bidi-isolate">{formatDateRange(visit.appointment.start_datetime, visit.appointment.end_datetime)}</dd></div><div><dt>{t("duration")}</dt><dd>{visit.appointment.duration_minutes} {t("minutes")}</dd></div><div><dt>{t("status")}</dt><dd><StatusBadge status={visit.status} /></dd></div><div className="detail-wide"><dt>{t("reason")}</dt><dd className="bidi-isolate">{displayText(visit.appointment.reason) || t("visitNotRecorded")}</dd></div><div><dt>{t("started")}</dt><dd className="bidi-isolate">{formatDateTime(visit.started_at)}</dd></div>{visit.completed_at ? <div><dt>{t("completed")}</dt><dd className="bidi-isolate">{formatDateTime(visit.completed_at)}</dd></div> : null}</dl></SurfaceCard><VisitBillingSection role={role} visit={visit} /></>;
}
...
export function VisitWorkspace({ role, visit }: VisitWorkspaceProps) {
  ...
  return <div className="visit-workspace"><SurfaceCard major className="visit-identity-card"><div className="visit-summary-header"><div><p className="visit-eyebrow">{t("activeVisit")}</p><h3 className="bidi-isolate">{visit.patient.full_name}</h3></div><div className="visit-status-region"><StatusBadge status={visit.status} /><span>{permissions.canEditClinicalNotes ? t("clinicalNotes") : t("readOnly")}</span></div></div><dl className="visit-context"><div><dt>{t("doctor")}</dt><dd className="bidi-isolate">{visit.doctor.full_name}</dd></div><div><dt>{t("started")}</dt><dd className="bidi-isolate">{formatDateTime(visit.started_at)}</dd></div>{visit.completed_at ? <div><dt>{t("completed")}</dt><dd className="bidi-isolate">{formatDateTime(visit.completed_at)}</dd></div> : null}</dl></SurfaceCard>
    <div className="visit-workspace-frame"><WorkspaceTabs tabs={tabs} defaultTab="notes" ariaLabel={t("visitTabs")} />
    {selected === "notes" ? <SurfaceCard className="visit-notes-card">{permissions.canEditClinicalNotes ? <><SectionHeading title={t("clinicalNotes")} description={t("clinicalNotesDescription")} /><ClinicalNotesForm values={values} isSaving={updateNotes.isPending} error={updateNotes.error} onChange={(field, value) => setValues((current) => ({ ...current, [field]: value }))} onSave={() => void saveNotes()} /></> : <ReadOnlyNotes values={values} />}</SurfaceCard> : null}
    ...
    </div><div className="visit-workspace-actions"><div><span>{permissions.canEditClinicalNotes ? (isDirty ? t("dirtyNotesComplete") : t("clinicalNotesDescription")) : t("clinicalNotesReadOnly")}</span></div><div className="visit-action-buttons"><Link className="v2-button secondary" to={`/${role.toLowerCase()}/patients/${visit.patient.id}`}>{t("patientProfile")}</Link>{permissions.canCompleteVisit ? <Button type="button" onClick={() => { updateNotes.reset(); completeVisit.reset(); setConfirmOpen(true); }}>{t("completeVisit")}</Button> : null}</div></div>
