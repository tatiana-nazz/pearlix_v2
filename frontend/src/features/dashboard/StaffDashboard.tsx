...
export function StaffDashboard() {
  ...
  return <main className="dashboard-v2" data-role="STAFF"><DashboardHeader language={language} clinicDate={data.clinic_date} clinicTimezone={data.clinic_timezone} title={c.staffTitle} description={c.staffDescription} refreshing={query.isFetching} onRefresh={() => void query.refetch()} />
    <DashboardMetrics><DashboardMetric icon={<CalendarDays size={22} />} label={c.appointments} value={data.today_appointments_count} to="/staff/appointments/day" /><DashboardMetric icon={<CalendarCheck2 size={22} />} label={c.checkedIn} value={data.checked_in_appointments.length} to="/staff/appointments/day" /><DashboardMetric icon={<CircleAlert size={22} />} label={c.needsReschedule} value={data.needs_reschedule_appointments.length} to="/staff/appointments/needs-reschedule" /><DashboardMetric icon={<ReceiptText size={22} />} label={c.unpaidInvoices} value={data.unpaid_or_partially_paid_invoices.length} to="/staff/billing/invoices" /></DashboardMetrics>
