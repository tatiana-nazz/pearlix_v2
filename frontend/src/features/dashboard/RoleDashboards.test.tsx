...
const adminData: AdminDashboardResponse = { clinic_date: "2026-08-08", clinic_timezone: "Asia/Damascus", today_appointments_count: 2, checked_in_appointments_count: 1, needs_reschedule_appointments_count: 3, active_visits_count: 2, open_bills_count: 7, partially_paid_bills_count: 2, today_invoices_count: 4, collected_today: { SYP: "150000.00", USD: "40.00" }, today_appointments: [appointment(4)], appointment_status_last_7_days: counts, billing_activity_last_30_days: activity, recent_handoffs: [handoff] };
...
describe("role dashboards", () => {
  ...
  it("routes Admin bill KPIs and recent obligations to Handoffs", async () => { setUser("ADMIN"); vi.spyOn(dashboardApi, "admin").mockResolvedValue(adminData); renderDashboard(<AdminDashboard />); expect(await screen.findByRole("link", { name: "Open bills: 7" })).toHaveAttribute("href", "/admin/billing/handoffs?status=OPEN"); expect(screen.getByRole("link", { name: "Partially paid bills 2" })).toHaveAttribute("href", "/admin/billing/handoffs?status=PARTIALLY_PAID"); expect(screen.getByRole("link", { name: "Bill 12: Maya Hassan" })).toHaveAttribute("href", "/admin/billing/handoffs/12"); expect(screen.getByLabelText(/SYP: Billed 300000.00, Collected 150000.00/)).toBeInTheDocument(); });
