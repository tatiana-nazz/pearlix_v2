...
export function CreateHandoffDialog({ error, pending, onCancel, onSubmit }: { error?: unknown; pending: boolean; onCancel: () => void; onSubmit: (payload: BillingHandoffCreatePayload) => void }) {
  ...
  return <Modal open wide title={t("createHandoff")} description={t("createHandoffDescription")} pending={pending} dirty={dirty} onClose={onCancel}><form className="billing-form clinical-notes-form" onSubmit={(event) => { event.preventDefault(); if (amount && !isPositiveMoney(amount)) { setValidation(t("suggestedAmountInvalid")); return; } setValidation(""); onSubmit({ ...(note.trim() ? { note: note.trim() } : {}), ...(amount ? { suggested_amount: amount, currency: currency as "USD" | "SYP" } : {}) }); }}><label className="v2-field">{t("note")}<textarea rows={3} disabled={pending} value={note} onChange={(event) => setNote(event.target.value)} /></label><label className="v2-field">{t("suggestedAmount")}<input inputMode="decimal" disabled={pending} value={amount} onChange={(event) => setAmount(event.target.value)} /></label><label className="v2-field">{t("currency")}<select value={currency} disabled={pending || !amount} onChange={(event) => setCurrency(event.target.value)}>{currencies}</select></label>{validation ? <p className="field-error" role="alert">{validation}</p> : null}<Error error={error} message={t("handoffCreateFailed")} /><div className="form-actions"><Button variant="secondary" type="button" disabled={pending} onClick={onCancel}>{t("cancel")}</Button><Button type="submit" loading={pending}>{t("create")}</Button></div></form></Modal>;
}
...
export function ConvertHandoffDialog({ suggestedAmount, suggestedCurrency, error, pending, onCancel, onSubmit }: { suggestedAmount: string | null; suggestedCurrency: "USD" | "SYP" | null; error?: unknown; pending: boolean; onCancel: () => void; onSubmit: (payload: HandoffConversionPayload) => void }) {
  ...
  return <Modal open title={t("convertHandoffTitle")} description={t("convertHandoffDescription")} pending={pending} dirty={dirty} onClose={onCancel}><form className="billing-form clinical-notes-form" onSubmit={(event) => { event.preventDefault(); if (!isPositiveMoney(amount)) { setValidation(t("totalAmountRequired")); return; } setValidation(""); onSubmit({ total_amount: amount, currency, ...(notes.trim() ? { notes: notes.trim() } : {}) }); }}><label className="v2-field">{t("totalAmount")}<input inputMode="decimal" disabled={pending} value={amount} onChange={(event) => setAmount(event.target.value)} /></label><label className="v2-field">{t("currency")}<select disabled={pending} value={currency} onChange={(event) => setCurrency(event.target.value as "USD" | "SYP")}>{currencies}</select></label><label className="v2-field">{t("notes")}<textarea rows={3} disabled={pending} value={notes} onChange={(event) => setNotes(event.target.value)} /></label>{validation ? <p className="field-error" role="alert">{validation}</p> : null}<Error error={error} message={t("handoffConvertFailed")} /><div className="form-actions"><Button variant="secondary" type="button" disabled={pending} onClick={onCancel}>{t("cancel")}</Button><Button type="submit" loading={pending}>{t("convertInvoice")}</Button></div></form></Modal>;
}
...
export function PaymentDialog({ currency, remaining, error, pending, onCancel, onSubmit }: { currency: "USD" | "SYP"; remaining: string; error?: unknown; pending: boolean; onCancel: () => void; onSubmit: (payload: PaymentPayload) => void }) {
  ...
  const dirty = Boolean(amount || notes);
  return <Modal open title={t("recordPayment")} description={`${t("remainingBalance")}: ${remaining} ${currency}.`} pending={pending} dirty={dirty} onClose={onCancel}><form className="billing-form clinical-notes-form" onSubmit={(event) => { event.preventDefault(); if (!isPositiveMoney(amount)) { setValidation(t("paymentAmountInvalid")); return; } if (Number(amount) > Number(remaining)) { setValidation(t("paymentExceedsBalance")); return; } setValidation(""); onSubmit({ amount, currency, ...(notes.trim() ? { notes: notes.trim() } : {}) }); }}><label className="v2-field">{t("amount")}<input inputMode="decimal" disabled={pending} value={amount} onChange={(event) => setAmount(event.target.value)} /></label><label className="v2-field">{t("currency")}<input value={currency} readOnly aria-readonly="true" /></label><label className="v2-field">{t("notes")}<textarea rows={3} disabled={pending} value={notes} onChange={(event) => setNotes(event.target.value)} /></label>{validation ? <p className="field-error" role="alert">{validation}</p> : null}<Error error={error} message={t("paymentFailed")} /><div className="form-actions"><Button variant="secondary" type="button" disabled={pending} onClick={onCancel}>{t("cancel")}</Button><Button type="submit" loading={pending}>{t("recordPayment")}</Button></div></form></Modal>;
}
...
export function InvoiceEditDialog({ invoice, error, pending, onCancel, onSubmit }: { invoice: Invoice; error?: unknown; pending: boolean; onCancel: () => void; onSubmit: (payload: InvoicePayload) => void }) {
  ...
  return <Modal open title={t("editInvoice")} description={t("editInvoiceDescription")} pending={pending} dirty={dirty} onClose={onCancel}><form className="billing-form clinical-notes-form" onSubmit={(event) => { event.preventDefault(); if (!paymentsLockAmounts && !isPositiveMoney(amount)) { setValidation(t("totalAmountRequired")); return; } const payload: InvoicePayload = {}; if (!paymentsLockAmounts && amount !== invoice.total_amount) payload.total_amount = amount; if (!paymentsLockAmounts && currency !== invoice.currency) payload.currency = currency; if (!notesLocked && notes !== invoice.notes) payload.notes = notes.trim(); setValidation(""); if (Object.keys(payload).length) onSubmit(payload); else onCancel(); }}><label className="v2-field">{t("totalAmount")}<input inputMode="decimal" disabled={pending || paymentsLockAmounts} value={amount} onChange={(event) => setAmount(event.target.value)} /></label><label className="v2-field">{t("currency")}<select disabled={pending || paymentsLockAmounts} value={currency} onChange={(event) => setCurrency(event.target.value as "USD" | "SYP")}>{currencies}</select></label><label className="v2-field">{t("notes")}<textarea rows={3} disabled={pending || notesLocked} value={notes} onChange={(event) => setNotes(event.target.value)} /></label>{validation ? <p className="field-error" role="alert">{validation}</p> : null}<Error error={error} message={t("invoiceUpdateFailed")} /><div className="form-actions"><Button variant="secondary" type="button" disabled={pending} onClick={onCancel}>{t("cancel")}</Button><Button type="submit" loading={pending} disabled={!dirty}>{t("save")}</Button></div></form></Modal>;
}
