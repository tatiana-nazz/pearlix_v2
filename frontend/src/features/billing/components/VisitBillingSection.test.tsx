...
const state = vi.hoisted(() => ({ handoffs: { data: { results: [] }, isLoading: false, isError: false, refetch: vi.fn() }, mutations: { createHandoff: { reset: vi.fn(), isPending: false, error: null, mutateAsync: vi.fn() } } }));
...
function view(role: "ADMIN" | "STAFF" | "DOCTOR") { return render(<QueryClientProvider client={new QueryClient()}><MemoryRouter><VisitBillingSection role={role} visit={visit} /></MemoryRouter></QueryClientProvider>); }
describe("VisitBillingSection production role boundary", () => { it("shows Doctor creation only for an owning completed visit", () => { useAuthStore.setState({ user: doctor }); view("DOCTOR"); expect(screen.getByRole("button", { name: "Create billing handoff" })).toBeInTheDocument(); }); it("keeps Admin and Staff read-only", () => { useAuthStore.setState({ user: doctor }); view("ADMIN"); expect(screen.queryByRole("button", { name: "Create billing handoff" })).not.toBeInTheDocument(); }); });
