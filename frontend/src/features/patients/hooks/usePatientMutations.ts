...
import type { CreatePatientPayload, UpdatePatientPayload } from "../../../types/patients";
...
export function useArchivePatient() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (patientId: number) => archivePatient(patientId),
    onSuccess: (patient) => invalidatePatients(queryClient, patient.id),
  });
}
...
export function useUnarchivePatient() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (patientId: number) => unarchivePatient(patientId),
    onSuccess: (patient) => invalidatePatients(queryClient, patient.id),
  });
}
