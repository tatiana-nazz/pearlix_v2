...
describe("PatientBillingSummary", () => {
  ...
  it("shows Handoff-first patient billing and the Staff New Bill route", () => { render(<MemoryRouter><PatientBillingSummary role="STAFF" patientId={44} /></MemoryRouter>); expect(screen.getByRole("link", { name: "New bill for patient" })).toHaveAttribute("href", "/staff/billing/handoffs/new?patient_id=44"); expect(screen.getByText("Preventive dental care")).toBeInTheDocument(); expect(screen.getByText("Outstanding USD").parentElement).toHaveTextContent(/USD\s*65\.00/); expect(screen.getByText("INV-20260808-000071")).toBeInTheDocument(); });
