...
export function AppointmentCalendarToolbar({ role, view, views, date, canCreate, onPrevious, onNext, onToday, onCreate }: { role: UserRole; view: AppointmentViewMode; views: AppointmentViewMode[]; date: string; canCreate: boolean; onPrevious: () => void; onNext: () => void; onToday: () => void; onCreate: () => void }) {
  const t = useFeatureT();
  ...
  return <section className="appointment-calendar-toolbar">
    ...
    {calendarView ? <div className="appointment-period-controls" aria-label={t("calendarNavigation")}>
      <button className="button secondary calendar-nav calendar-nav-prev" type="button" aria-label={previousLabel} title={previousLabel} onClick={onPrevious}><span aria-hidden="true">‹</span></button>
      ...
      <button className="button secondary calendar-nav calendar-nav-next" type="button" aria-label={nextLabel} title={nextLabel} onClick={onNext}><span aria-hidden="true">›</span></button>
