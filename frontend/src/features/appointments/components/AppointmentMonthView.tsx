  ...
  return (
    <div className="appointment-calendar-scroll" data-calendar-scroll="month"><div className="appointment-month-grid"><div className="appointment-month-weekdays">{weekdays.map((weekday) => <span key={weekday}>{weekday}</span>)}</div>
      {days.map((day) => {
        ...
        return (
          <section key={day} className={`${isDateInMonth(day, date) ? "appointment-month-cell" : "appointment-month-cell muted"} ${day === date ? "selected" : ""} ${day === today ? "today" : ""}`}>
            <div className="appointment-month-cell-heading"><button className="appointment-month-day-header" type="button" aria-label={`${day === date ? `${t("selectedDay")}: ` : ""}${t("viewDay")}: ${formatDate(`${day}T00:00:00`) || day}`} onClick={() => onSelectDay(day)} onDoubleClick={() => onOpenDay(day)}><h3>{Number(day.slice(8, 10))}</h3></button><span aria-label={`${dayAppointments.length} ${t("calendarAppointmentCount")}`}>{dayAppointments.length}</span></div>
            {dayAppointments.slice(0, 3).map((appointment) => (
              <button key={appointment.id} type="button" className={appointmentRecordClass(appointment.status)} onClick={(event) => { event.stopPropagation(); onDetails(appointment); }}>
                <span className="bidi-isolate">{formatTime(appointment.start_datetime)} {appointment.patient.full_name}</span><AppointmentStatusBadge status={appointment.status} />
              </button>
            ))}
