...
| PAT-14D4A | patient directory/create/detail | server paging/filter state, Staff-only General Information create, read-first profile, explicit medical-history edit, version/archive boundaries, centralized EN/AR copy, canonical broad active-Doctor scope | direct payload/dirty-cancel/row-keyboard/tab/fallback/invalidation tests; browser evidence pending | 14D.4A |
