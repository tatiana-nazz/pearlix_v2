...
The DEBUG-only `seed_demo_clinic_story --reset-demo --reference-date 2026-07-26` supplied disposable saved, external, attached, discarded, stored-result, and no-result records. Browser checks confirmed Doctor and Staff saved-X-ray action-free collections, detail metadata/AI disclaimer/no Run AI control, Staff read-only boundaries, and Admin's action-free external collection with its existing upload entry point. A direct authenticated protected-media request returned `200 image/png` with the synthetic fixture. The in-app browser used for this evidence did not implement Blob object URLs and therefore rendered the viewer's protected-media failure state instead of an image; this is recorded as an environment limitation rather than a backend/security change. The seed was reset after QA and no media, database, screenshot, trace, credential, log, or generated artifact is tracked.

## Known limitations

Real AI remains unavailable. Existing backend mock-adapter records are displayed only as stored, labelled supportive information; no real inference is claimed. Browser visual acceptance of the protected image and overlay still needs an object-URL-capable browser surface before release acceptance can be closed. This record is implementation evidence, not product authority.
