...
Arabic font stack is `"Noto Sans Arabic", Tahoma, Arial, sans-serif` (no font bundle required); use 1.75 body, 1.55 label, and 1.35 heading line-height. Latin uses Inter/system UI. Apply `unicode-bidi:isolate` plus LTR direction to emails, phone numbers, IDs, dates, invoice numbers, and currency/amounts; mixed-script names wrap at words and clamp only after two lines in cards. Use `text-align:start`, logical gaps/padding, and preserve copyable untruncated values in tables. Mirroring exceptions are Close, product mark, X-ray/scan image, status icons, and non-directional controls.
