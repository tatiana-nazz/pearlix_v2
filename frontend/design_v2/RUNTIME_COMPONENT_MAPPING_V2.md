...
`pages/dashboard/DashboardV2.tsx` now supplies v2 KPI and bounded preview composition for all role dashboards; dashboard legacy summary cards remain only where their API payload has no corresponding queue. `AppointmentsPage.tsx` owns v2 centered Modal/ConfirmDialog behavior directly; appointment Details, Add/Edit/Reschedule, and status confirmations no longer use drawers. `PatientTable.tsx` now uses `ClickableRow` and has no routine View action, while patient edit/archive consumers use v2 overlays. `pages/admin/TeamPages.tsx` is the distinct Team surface; `AdminManagementPages.tsx` now owns the signed Users & Access transition flow.
