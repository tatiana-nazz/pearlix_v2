...
# Phase 14D browser acceptance mapping update

The role shell maps translated navigation labels, localized workspace metadata, and localized sidebar control labels through `src/layouts/i18n.ts` and `src/layouts/Sidebar.tsx`. The responsive shell owns document-overflow clipping in `src/layouts/Shell.css`. Browser acceptance evidence is indexed by `QA_14D_BROWSER_ACCEPTANCE.md` and `PHASE_14D_BROWSER_ACCEPTANCE_RECORD.md`.
